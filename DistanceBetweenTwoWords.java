...
public class DistanceBetweenTwoWords {
	...
	public static double shortestDistance(String document, String word1, String word2) {
		...
		int countWord = 0;
		double countOneWord = 0, countTwoWord = 0;
		String[] words = document.split("[,. ]");

		for (String word : words) {
			if (word.equalsIgnoreCase(word1))
				countOneWord = countWord + word.length() / 2;
			if (word.equalsIgnoreCase(word2))
				countTwoWord = countWord + word.length() / 2;
			if (countOneWord > 0 && countTwoWord > 0)
				shortest = Math.min(shortest, Math.abs(countTwoWord - countOneWord));
			countWord = countWord + word.length() + 1;
		}
		return shortest;
	}
