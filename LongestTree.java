...
class LongestTree
{
  ...
  */
  ...
  {
	  Map<Integer, List<Integer>> parentToChild = new HashMap<Integer, List<Integer>>();
	  immediateParent.forEach((k,v)->{
		  if(parentToChild.containsKey(v)) {
			  parentToChild.get(v).add(k);
		  }
		  else {
			  List<Integer> children = new ArrayList<Integer>();
			  children.add(k);
			  parentToChild.put(v, children);
		  }
	  });
	  Integer length = null;
	  Integer value = null;
	  for(Integer parent: parentToChild.keySet()) {
		  Integer result = getTreeSize(parentToChild, parent);
		  ...
		  else if(result == length && parent<value) {
			  value = parent;
			  length = result;
		  }
	  }
